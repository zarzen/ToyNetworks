...
class Network(object):
    ...
    def update_mini_batch(self, mini_batch, eta, v_b_pre, v_w_pre):
        ...
        # update velocity
        v_b = [self.momentum * v_pre - (eta/len(mini_batch)) * nb
                   for v_pre, nb in zip(v_b_pre, nabla_b)]
        v_w = [self.momentum * v_pre - (eta/len(mini_batch)) * nw
                   for v_pre, nw in zip(v_w_pre, nabla_w)]
        # momentum update
        # if momentum is 0, it equals to vanilla update
        self.weights = [w + v for w, v in zip(self.weights, v_w)]
...
def main():
    ...
    lr = 1
    mu = 0.6
    print('settings: batch_size {0}; learning rate {1}; momentum: {2}'.format(
        batch_size, lr, mu
    ))
    ...
    model_vanilla.SGD(train, n_epoch, batch_size, lr, val)
    ...
    model_momentum.SGD(train, n_epoch, batch_size, lr, val)
    ...
    model_nesterov.SGD(train, n_epoch, batch_size, lr, val)
